...
function InitBtnGroupBtm(bNeebBack) {
    ...
    if (btnGroupBtm) {
		...
		objString += "<div class='ct-rt'>Visualizar: ";
...
function initCustomScan() {
    ...
    ObjString += "<label for = 'ADF'><input type='checkbox' id='ADF' />Automático &nbsp;</label>";
    ObjString += "<label for = 'Duplex'><input type='checkbox' id='Duplex'/>Doble</label></li>";
    ObjString += "<li>Tipo de pixeles:";
    ObjString += "<label for='BW' style='margin-left:5px;'><input type='radio' id='BW' name='PixelType'/>B&amp;N </label>";
    ObjString += "<label for='Gray'><input type='radio' id='Gray' name='PixelType'/>Plomo</label>";
    ObjString += "<label for='RGB'><input type='radio' id='RGB' name='PixelType'/>Color</label></li>";
    ObjString += "<li>";
    ObjString += "<span>Resolución:</span><select size='1' id='Resolution'><option value = ''></option></select></li>";
    ObjString += "</ul>";
...
function setDefaultValue() {
    ...
    /*aca le manda el nombre del documento*/
...
function checkErrorStringWithErrorCode(errorCode, errorString, responseString) {
    if (errorCode == 0) {
        appendMessage("<span style='color:green'><b>" + errorString + "</b></span><br />");
